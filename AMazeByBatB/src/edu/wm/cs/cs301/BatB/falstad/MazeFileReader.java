 ...
 *
 */
public class MazeFileReader extends AsyncTask<Void,Void,Integer>{
	 ...
	 */
	public MazeFileReader(String filename, Maze maze) {
		this.filename = filename;
		MazeFileReader.maze = maze;
		MazeFileReader.panel = maze.getGUIWrapper();
		this.execute();
	}
	...
	protected Integer doInBackground(Void... params) {
		System.out.println("Got into doInBackground");
		load(filename);
		return 0;
	}
	
	protected void onPostExecute(Integer returnVal){
		System.out.println("Called onPostExecute");
		maze.getGeneratingActivity().finishLoadingMaze();;
	...
	{
		try{
			System.out.println("Got into load");
	...
	private static Seg readSegment(Element eElement, int number, int i) {
		...
		result.col = result.decipherSegColorValue(col);
	...
	private static void compareSegments(ArrayList<Seg> slist, ArrayList<Seg> slist2) {
		...
		for (int i = 0 ; i < n ; i++)
		{
			...
			if(panel.getRGB(s.col, s.val1) != panel.getRGB(s.col, s.val1)) 
